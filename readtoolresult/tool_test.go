...
func TestPrepareRefusesMalformedArguments(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "capture id in another case", args: `{"Capture_ID":"` + captureID + `"}`},
		{name: "offset in another case", args: `{"capture_id":"` + captureID + `","OFFSET":1}`},
		{name: "case-variant duplicate", args: `{"capture_id":"` + captureID + `","CAPTURE_ID":"00000000-0000-4000-8000-000000000000"}`},
		{name: "escaped name variant", args: `{"capture\u005fid":"` + captureID + `","Capture_id":"` + captureID + `"}`},
